...
ask_for_updates() {
    ...
    if [[ -n "${marmot_UPDATE_AVAILABLE:-}" && "${marmot_UPDATE_AVAILABLE}" == "true" ]]; then
        has_updates=true
        update_list+=("  • marmot")
    fi
...
perform_updates() {
    ...
    [[ -n "${marmot_UPDATE_AVAILABLE:-}" && "${marmot_UPDATE_AVAILABLE}" == "true" ]] && ((total_count++))
    ...
    # Update marmot
    if [[ -n "${marmot_UPDATE_AVAILABLE:-}" && "${marmot_UPDATE_AVAILABLE}" == "true" ]]; then
        echo -e "${BLUE}Updating marmot...${NC}"
        if "${SCRIPT_DIR}/marmot" update 2>&1 | grep -qE "(Updated|latest version)"; then
            echo -e "${GREEN}✓${NC} marmot updated"
            reset_marmot_cache
            ((updated_count++))
        else
            echo -e "${RED}✗${NC} marmot update failed"
        fi
        echo ""
    fi
...
_perform_appstore_update() {
    ...
    local appstore_log
    appstore_log=$(mktemp -t marmot-appstore 2> /dev/null || echo "/tmp/marmot-appstore.log")
...
_perform_macos_update() {
    ...
    local macos_log
    macos_log=$(mktemp -t marmot-macos 2> /dev/null || echo "/tmp/marmot-macos.log")
