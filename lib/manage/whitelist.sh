...
_marmot_MANAGE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$_marmot_MANAGE_DIR/../core/common.sh"
source "$_marmot_MANAGE_DIR/../ui/menu_simple.sh"

# Config file paths
readonly WHITELIST_CONFIG_CLEAN="$HOME/.config/marmot/whitelist"
readonly WHITELIST_CONFIG_OPTIMIZE="$HOME/.config/marmot/whitelist_optimize"
readonly WHITELIST_CONFIG_OPTIMIZE_LEGACY="$HOME/.config/marmot/whitelist_checks"
...
manage_whitelist_categories() {
    ...
    if [[ ${#selected_cache_patterns[@]} -gt 0 ]]; then
        ...
        local IFS=','
        export marmot_PRESELECTED_INDICES="${preselected_indices[*]}"
    else
        unset marmot_PRESELECTED_INDICES
    fi
    ...
    if [[ -n "$marmot_SELECTION_RESULT" ]]; then
        local -a selected_indices
        IFS=',' read -ra selected_indices <<< "$marmot_SELECTION_RESULT"
