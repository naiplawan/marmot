...
paginated_multi_select() {
    ...
    if [[ "${marmot_MANAGED_ALT_SCREEN:-}" == "1" || "${marmot_MANAGED_ALT_SCREEN:-}" == "true" ]]; then
    ...
    local sort_mode="${marmot_MENU_SORT_DEFAULT:-date}" # date|name|size
    ...
    if [[ -n "${marmot_MENU_META_EPOCHS:-}" ]]; then
        while IFS= read -r v; do epochs+=("${v:-0}"); done < <(_pm_parse_csv_to_array "$marmot_MENU_META_EPOCHS")
        has_metadata="true"
    fi
    if [[ -n "${marmot_MENU_META_SIZEKB:-}" ]]; then
        while IFS= read -r v; do sizekb+=("${v:-0}"); done < <(_pm_parse_csv_to_array "$marmot_MENU_META_SIZEKB")
        has_metadata="true"
    fi
    ...
    if [[ -n "${marmot_PRESELECTED_INDICES:-}" ]]; then
        local cleaned_preselect="${marmot_PRESELECTED_INDICES//[[:space:]]/}"
    ...
    cleanup() {
        trap - EXIT INT TERM
        restore_terminal
        unset marmot_READ_KEY_FORCE_CHAR
    }
        ...
        case "$key" in
            ...
            "FILTER")
                ...
                if [[ -n "$applied_query" ]]; then
                    ...
                    # Enter filter mode
                    filter_mode="true"
                    export marmot_READ_KEY_FORCE_CHAR=1
                    filter_query=""
                    top_index=0
                    cursor_pos=0
                    rebuild_view
            ...
            "ENTER")
                ...
                trap - EXIT INT TERM
                marmot_SELECTION_RESULT="$final_result"
                restore_terminal
                return 0
                ;;
        esac
