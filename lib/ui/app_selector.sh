...
select_apps_for_uninstall() {
    ...
    # - marmot_MENU_META_EPOCHS: numeric last_used_epoch per item
    # - marmot_MENU_META_SIZEKB: numeric size in KB per item
    # The menu will gracefully fallback if these are unset or malformed.
    export marmot_MENU_META_EPOCHS="$epochs_csv"
    export marmot_MENU_META_SIZEKB="$sizekb_csv"
    # Optional: allow default sort override via env (date|name|size)
    # export marmot_MENU_SORT_DEFAULT="${marmot_MENU_SORT_DEFAULT:-date}"

    # Use paginated menu - result will be stored in marmot_SELECTION_RESULT
    ...
    unset marmot_MENU_META_EPOCHS marmot_MENU_META_SIZEKB
    # leave marmot_MENU_SORT_DEFAULT untouched if user set it globally
    ...
    if [[ -z "$marmot_SELECTION_RESULT" ]]; then
    ...
    IFS=',' read -r -a indices_array <<< "$marmot_SELECTION_RESULT"
