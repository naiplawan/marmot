...
check_marmotle_update() {
    ...
    if is_cache_valid "$cache_file"; then
        ...
        # Show spinner while checking
        if [[ -t 1 ]]; then
            start_inline_spinner "Checking marmot version..."
        fi
    ...
    if [[ -n "$latest_version" && "$current_version" != "$latest_version" ]]; then
        ...
        if [[ "$(printf '%s\n' "$current_version" "$latest_version" | sort -V | head -1)" == "$current_version" ]]; then
            export marmot_UPDATE_AVAILABLE="true"
            echo -e "  ${YELLOW}${ICON_WARNING}${NC} marmot         ${YELLOW}${latest_version} available${NC} (current: ${current_version})"
            echo -e "    ${GRAY}Run: ${GREEN}marmot update${NC}"
        else
            echo -e "  ${GREEN}✓${NC} marmot         Up to date (${current_version})"
        fi
    else
        echo -e "  ${GREEN}✓${NC} marmot         Up to date (${current_version})"
    fi
...
check_memarmotry_usage() {
    ...
    if [[ -z "$mem_total" || "$mem_total" -le 0 ]]; then
        echo -e "  ${GRAY}-${NC} Memarmotry       Unable to determine"
        return
    fi
    ...
    if [[ $used_percent -gt 90 ]]; then
        echo -e "  ${RED}✗${NC} Memarmotry       ${RED}${used_percent}% used${NC} (Critical)"
    elif [[ $used_percent -gt 80 ]]; then
        echo -e "  ${YELLOW}${ICON_WARNING}${NC} Memarmotry       ${YELLOW}${used_percent}% used${NC} (High)"
    else
        echo -e "  ${GREEN}✓${NC} Memarmotry       ${used_percent}% used"
    fi
}
...
check_login_items() {
    ...
    if ((login_items_count > preview_limit)); then
        ...
        items_display="${items_display}, and ${remaining} marmotre"
    fi
