#!/bin/bash
# marmot - Timeout Control
# Command execution with timeout support

set -euo pipefail

# Prevent multiple sourcing
if [[ -n "${marmot_TIMEOUT_LOADED:-}" ]]; then
    return 0
fi
readonly marmot_TIMEOUT_LOADED=1
...
if [[ -z "${MAR_TIMEOUT_INITIALIZED:-}" ]]; then
    MAR_TIMEOUT_BIN=""
    for candidate in gtimeout timeout; do
        if command -v "$candidate" > /dev/null 2>&1; then
            MAR_TIMEOUT_BIN="$candidate"
            if [[ "${MAR_DEBUG:-0}" == "1" ]]; then
...
#
# Environment:
#   MAR_DEBUG - Set to 1 to enable debug logging to stderr
#
...
run_with_timeout() {
    ...
    if [[ -n "${MAR_TIMEOUT_BIN:-}" ]]; then
        if [[ "${MAR_DEBUG:-0}" == "1" ]]; then
            echo "[TIMEOUT] Running with ${duration}s timeout: $*" >&2
        fi
        "$MAR_TIMEOUT_BIN" "$duration" "$@"
        return $?
    fi
