#!/bin/bash
# marmot - Base Definitions and Utilities
# Core definitions, constants, and basic utility functions used by all modules

set -euo pipefail

# Prevent multiple sourcing
if [[ -n "${marmot_BASE_LOADED:-}" ]]; then
    return 0
fi
readonly marmot_BASE_LOADED=1
...
# ============================================================================
readonly marmot_TEMP_FILE_AGE_DAYS=7       # Temp file cleanup threshold
readonly marmot_ORPHAN_AGE_DAYS=60         # Orphaned data threshold
readonly marmot_MAX_PARALLEL_JOBS=15       # Parallel job limit
readonly marmot_MAIL_DOWNLOADS_MIN_KB=5120 # Mail attachments size threshold
readonly marmot_LOG_AGE_DAYS=7             # System log retention
readonly marmot_CRASH_REPORT_AGE_DAYS=7    # Crash report retention
readonly marmot_SAVED_STATE_AGE_DAYS=7     # App saved state retention
readonly marmot_TM_BACKUP_SAFE_HOURS=48    # Time Machine failed backup safety window
...
declare -a marmot_TEMP_FILES=()
declare -a marmot_TEMP_DIRS=()
...
cleanup_temp_files() {
    local file
    if [[ ${#marmot_TEMP_FILES[@]} -gt 0 ]]; then
        for file in "${marmot_TEMP_FILES[@]}"; do
            [[ -f "$file" ]] && rm -f "$file" 2> /dev/null || true
        done
    fi

    if [[ ${#marmot_TEMP_DIRS[@]} -gt 0 ]]; then
        for file in "${marmot_TEMP_DIRS[@]}"; do
