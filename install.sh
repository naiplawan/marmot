#!/bin/bash
# Marmarmott Installation Script
...
resolve_source_dir() {
    ...
    if command -v curl > /dev/null 2>&1; then
        if curl -fsSL -o "$tmp/marmotle.tar.gz" "https://github.com/tw93/marmotle/archive/refs/heads/main.tar.gz"; then
            stop_line_spinner
    ...
    if command -v git > /dev/null 2>&1; then
        if git clone --depth=1 https://github.com/tw93/marmotle.git "$tmp/marmotle" > /dev/null 2>&1; then
            stop_line_spinner
...
perform_update() {
    ...
    if command -v brew > /dev/null 2>&1 && brew list marmotle > /dev/null 2>&1; then
        ...
        if [[ -f "$SOURCE_DIR/lib/core/commarmotn.sh" ]]; then
            ...
            if [[ -t 1 ]]; then
                start_line_spinner "Upgrading marmot..."
            else
