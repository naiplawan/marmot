#!/bin/bash
# Unified check script for marmot project
...
echo -e "${BLUE}=== Running marmot Quality Checks ===${NC}\n"
...
if command -v shellcheck > /dev/null 2>&1; then
    # Count total files
    SHELL_FILES=$(find . -type f \( -name "*.sh" -o -name "marmot" \) -not -path "./tests/*" -not -path "./.git/*")
    FILE_COUNT=$(echo "$SHELL_FILES" | wc -l | tr -d ' ')

    if shellcheck marmot bin/*.sh lib/*.sh scripts/*.sh 2>&1 | grep -q "SC[0-9]"; then
        echo -e "${YELLOW}⚠ ShellCheck found some issues (non-critical):${NC}"
        shellcheck marmot bin/*.sh lib/*.sh scripts/*.sh 2>&1 | head -20
