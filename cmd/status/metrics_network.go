...
func collectProxy() ProxyStatus {
	...
	// Linux: Check for system proxy via gsettings (GNOME) or environment variables
	if runtime.GOOS == "linux" {
		if commandExists("gsettings") {
			ctx, cancel := context.WithTimeout(context.Background(), 500*time.Millisecond)
			defer cancel()
			// Check HTTP proxy
			if out, err := runCmd(ctx, "gsettings", "get", "org.gnome.system.proxy.http", "host"); err == nil {
				host := strings.Trim(strings.TrimSpace(out), "'")
				if host != "" && host != "''" {
					// Get port
					if portOut, err := runCmd(ctx, "gsettings", "get", "org.gnome.system.proxy.http", "port"); err == nil {
						portStr := strings.Trim(strings.TrimSpace(portOut), "'")
						if portStr != "" && portStr != "0" {
							return ProxyStatus{Enabled: true, Type: "System", Host: host + ":" + portStr}
						}
					}
				}
			}
			// Check SOCKS proxy
			if out, err := runCmd(ctx, "gsettings", "get", "org.gnome.system.proxy.socks", "host"); err == nil {
				host := strings.Trim(strings.TrimSpace(out), "'")
				if host != "" && host != "''" {
					// Get port
					if portOut, err := runCmd(ctx, "gsettings", "get", "org.gnome.system.proxy.socks", "port"); err == nil {
						portStr := strings.Trim(strings.TrimSpace(portOut), "'")
						if portStr != "" && portStr != "0" {
							return ProxyStatus{Enabled: true, Type: "SOCKS", Host: host + ":" + portStr}
						}
					}
				}
			}
