...
var foldDirs = map[string]bool{
	...
	// Linux-specific cache and temporary directories
	".local":                 true, // ~/.local/share, ~/.local/state
	".config":                true, // Config directory with caches
	"cache":                  true, // Various cache directories
	".cache-var":              true, // /var/cache (distinguished from ~/.cache)
	"logs":                   true, // Log directories
...
var skipSystemDirs = map[string]bool{
	// Common system directories (both platforms)
	"dev":                     true,
	"tmp":                     true,
	"private":                 true,
	"cores":                   true,
	...
	"boot":     true, // /boot bootloader files
	"lib":      true, // /lib system libraries
	"lib64":    true, // /lib64 system libraries
	"usr":      true, // /usr (skip at root to avoid huge scan)
	"opt":      true, // /opt optional software
	"srv":      true, // /srv service data
	"lost+found": true, // Filesystem recovery directory
	"mnt":      true, // Mount point (empty by default)
	"media":    true, // Media mount point (empty by default)
}
