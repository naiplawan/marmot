...
func createOverviewEntries() []dirEntry {
	...
	if home != "" {
		if runtime.GOOS == "darwin" {
			...
			// Linux paths following XDG specification
			entries = append(entries,
				dirEntry{Name: "Home (~)", Path: home, IsDir: true, Size: -1},
				dirEntry{Name: "Documents", Path: filepath.Join(home, "Documents"), IsDir: true, Size: -1},
				dirEntry{Name: "Downloads", Path: filepath.Join(home, "Downloads"), IsDir: true, Size: -1},
				dirEntry{Name: "Videos", Path: filepath.Join(home, "Videos"), IsDir: true, Size: -1},
				dirEntry{Name: "Music", Path: filepath.Join(home, "Music"), IsDir: true, Size: -1},
				dirEntry{Name: "Pictures", Path: filepath.Join(home, "Pictures"), IsDir: true, Size: -1},
				dirEntry{Name: ".cache", Path: filepath.Join(home, ".cache"), IsDir: true, Size: -1},
				dirEntry{Name: ".local/share", Path: filepath.Join(home, ".local", "share"), IsDir: true, Size: -1},
			)
		}
	}
	...
	if runtime.GOOS == "darwin" {
		...
		entries = append(entries,
			dirEntry{Name: "Applications (/usr/bin)", Path: "/usr/bin", IsDir: true, Size: -1},
			dirEntry{Name: "System (/)", Path: "/", IsDir: true, Size: -1},
		)

		// Add mount points for external drives
		if hasUsefulVolumeMounts("/mnt") {
			entries = append(entries, dirEntry{Name: "Mounts", Path: "/mnt", IsDir: true, Size: -1})
		}
		if hasUsefulVolumeMounts("/media") {
			entries = append(entries, dirEntry{Name: "Media", Path: "/media", IsDir: true, Size: -1})
		}

		// Add Snap applications if present
		if _, err := os.Stat("/snap"); err == nil {
			entries = append(entries, dirEntry{Name: "Snap Apps", Path: "/snap", IsDir: true, Size: -1})
		}
	}
