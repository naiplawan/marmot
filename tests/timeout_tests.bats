...
setup() {
    ...
    export PROJECT_ROOT
    export MAR_DEBUG=0  # Disable debug output for cleaner tests
}
...
@test "run_with_timeout: debug logging when MAR_DEBUG=1" {
    output=$(bash -c "
        set -euo pipefail
        export MAR_DEBUG=1
        source '$PROJECT_ROOT/lib/core/timeout.sh'
...
@test "run_with_timeout: no debug logging when MAR_DEBUG=0" {
    # When MAR_DEBUG=0, no debug messages should appear during function execution
    # (Initialization messages may appear if module is loaded for first time)
    output=$(bash -c "
        set -euo pipefail
        export MAR_DEBUG=0
        unset MAR_TIMEOUT_INITIALIZED  # Force re-initialization
        source '$PROJECT_ROOT/lib/core/timeout.sh'
...
@test "timeout.sh: sets marmot_TIMEOUT_LOADED flag" {
    result=$(bash -c "
        set -euo pipefail
        source '$PROJECT_ROOT/lib/core/timeout.sh'
        echo \"\$marmot_TIMEOUT_LOADED\"
    ")
    [[ "$result" == "1" ]]
}
