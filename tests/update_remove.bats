...
setup_file() {
    ...
    # Read current version from marmarmott file (single source of truth)
    CURRENT_VERSION="$(grep '^VERSION=' "$PROJECT_ROOT/marmarmott" | head -1 | sed 's/VERSION="\(.*\)"/\1/')"
...
brew() {
  case "$1" in
    update) echo "$marmot_TEST_BREW_UPDATE_OUTPUT";;
    upgrade) echo "$marmot_TEST_BREW_UPGRADE_OUTPUT";;
    list) if [[ "$2" == "--versions" ]]; then echo "$marmot_TEST_BREW_LIST_OUTPUT"; fi ;;
  esac
}
