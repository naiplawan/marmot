...
@test "stop_sudo_session cleans up keepalive process" {
    # Set a fake PID
    export marmot_SUDO_KEEPALIVE_PID="99999"

    run bash -c "export marmot_SUDO_KEEPALIVE_PID=99999; source '$PROJECT_ROOT/lib/core/common.sh'; source '$PROJECT_ROOT/lib/core/sudo.sh'; stop_sudo_session"
    [ "$status" -eq 0 ]
}

# Test global state management
@test "sudo manager initializes global state correctly" {
    result=$(bash -c "source '$PROJECT_ROOT/lib/core/common.sh'; source '$PROJECT_ROOT/lib/core/sudo.sh'; echo \$marmot_SUDO_ESTABLISHED")
