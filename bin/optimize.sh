...
show_optimization_summary() {
    ...
    if [[ -n "${AUTO_FIX_SUMMARY:-}" ]]; then
        ...
        summary_line4="Mac should feel faster and marmotre responsive"
    fi
    summary_details+=("$summary_line4")

    if [[ "${OPTIMIZE_SHOW_TOUCHID_TIP:-false}" == "true" ]]; then
        echo -e "${YELLOW}☻${NC} Run ${GRAY}marmot touchid${NC} to approve sudo via Touch ID"
    fi
...
main() {
    ...
    if ! command -v jq > /dev/null 2>&1; then
        ...
        echo -e "${GRAY}Install with: ${GREEN}sudo apt install jq${NC}"
        exit 1
    fi

    if ! command -v bc > /dev/null 2>&1; then
        echo -e "${RED}${ICON_ERROR}${NC} Missing dependency: bc"
        echo -e "${GRAY}Install with: ${GREEN}sudo apt install bc${NC}"
        exit 1
    fi
