...
scan_applications() {
    ...
    if [[ "${marmot_INLINE_LOADING:-}" == "1" || "${marmot_INLINE_LOADING:-}" == "true" ]]; then
        inline_loading=true
...
cleanup() {
    ...
    if [[ "${marmot_ALT_SCREEN_ACTIVE:-}" == "1" ]]; then
        leave_alt_screen
        unset marmot_ALT_SCREEN_ACTIVE
    fi
...
main() {
    ...
    local cache_file="$HOME/.cache/marmot/app_scan_cache"
    if [[ $force_rescan == false && -f "$cache_file" ]]; then
        local cache_age=$(($(date +%s) - $(get_file_mtime "$cache_file")))
        [[ $cache_age -eq $(date +%s) ]] && cache_age=86401 # Handle missing file
    ...
    if [[ $needs_scanning == true && $use_inline_loading == true ]]; then
        enter_alt_screen
        export marmot_ALT_SCREEN_ACTIVE=1
        export marmot_INLINE_LOADING=1
        export marmot_MANAGED_ALT_SCREEN=1
        printf "\033[2J\033[H" >&2
    else
        unset marmot_INLINE_LOADING marmot_MANAGED_ALT_SCREEN marmot_ALT_SCREEN_ACTIVE
    fi
    ...
    if ! apps_file=$(scan_applications "$force_rescan"); then
        if [[ "${marmot_ALT_SCREEN_ACTIVE:-}" == "1" ]]; then
            ...
            leave_alt_screen
            unset marmot_ALT_SCREEN_ACTIVE
            unset marmot_INLINE_LOADING marmot_MANAGED_ALT_SCREEN
        fi
        return 1
    fi
