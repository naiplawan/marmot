#!/bin/bash
# Marmarmott - Deeper system cleanup
...
safe_clean() {
    ...
    for path in "${targets[@]}"; do
        ...
        case "$path" in
            *clash* | *Clash* | *surge* | *Surge* | *mihomarmot* | *openvpn* | *OpenVPN*)
                skip=true
                ((skipped_count++))
                ;;
        esac
    ...
    if [[ ${#existing_paths[@]} -gt 3 ]]; then
        ...
        if [[ -t 1 ]]; then marmot_SPINNER_PREFIX="  " start_inline_spinner "Scanning $total_paths items..."; fi
        local temp_dir
        ...
        for path in "${existing_paths[@]}"; do
            ...
            if ((${#pids[@]} >= marmot_MAX_PARALLEL_JOBS)); then
                ...
                if [[ -t 1 ]] && ((completed % 20 == 0)); then
                    stop_inline_spinner
                    marmot_SPINNER_PREFIX="  " start_inline_spinner "Scanning items ($completed/$total_paths)..."
                fi
            fi
        done
...
start_cleanup() {
    ...
    if [[ "$DRY_RUN" != "true" && -t 0 ]]; then
        echo -e "${YELLOW}☻${NC} First time? Run ${GRAY}marmot clean --dry-run${NC} first to preview changes"
    fi
...
perform_cleanup() {
    ...
    if [[ $total_size_cleaned -gt 0 ]]; then
        ...
        if [[ "$DRY_RUN" == "true" ]]; then
            ...
            summary_details+=("Use ${GRAY}marmot clean --whitelist${NC} to add protection rules")
        else
